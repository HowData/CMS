...
public class StringUtil
{
    ...
    public static String transFileName(String filename)
    {
        ...
        StringUtil.strIfNull(ConstantUtils.obj.getSysEnvironment().get("FileBeforeTransEncode"));
        String afterTransEncode =
        StringUtil.strIfNull(ConstantUtils.obj.getSysEnvironment().get("FileAfterTransEncode"));
    ...
    /** 对于字符串为空的判断处理
     *
     * @param src String 要判断的原始字符串
     * @param defaultValue String
     * 假如原始字符串为空的时候需要返回的默认字符串，假如该参数为空并且原始字符串为空返回长度为零的字符串
     * @param isTrim boolean 是否需要截断空格
     * @return String
     * @author huangjiansong */
    public static String strIfNull(String src,String defaultValue,boolean isTrim)
    {
        if (src == null)
            src = "";
        if (isTrim)
     ...
     * @param src String
     * @return String */
    public static String strIfNull(String src)
    {
        if (src == null)
            src = "";
        return src;
     ...
     * @param src String
     * @return String */
    public static String strChangeZeroByDouble(String src)
    {
        if (src == null)
            src = "";
    ...
    public static String fromBlob(Blob blob)
    {
        ...
        try
        {
            ...
            if (alllen > 0 && v.size() > 0)
            {
                ...
                ConstantUtils.obj.getSysEnvironment().get("afterTransEncode.get").toString());
            }
        }
        catch(Exception e)
        {
    ...
    public static String getHumanFilesize(long filesize)
    {
        ...
        {
            s = Integer.toString(Math.round(filesize / (1024 * 1024 * 1024) * 100) / 100) + "Gb";
        }
        else if (filesize >= (1024 * 1024))
        {
            s = Integer.toString(Math.round(filesize / (1024 * 1024) * 100) / 100) + "Mb";
        }
        else if (filesize >= 1024)
        {
            s = Integer.toString(Math.round(filesize / 1024 * 100) / 100) + "Kb";
        }
        else if (filesize > 0)
        {
